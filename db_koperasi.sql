...
(1,'TV01','Televisi 21 inch',3500000,2400000,15,2,1),
(2,'TV02','Televisi 40 inch',5500000,7440000,44,2,1),
...
CREATE TABLE `vendor` (
  ...
  UNIQUE KEY `idx_unique_vendor` (`nama`,`kota`,`kontak`),
...
--
-- Temporary table structure for view `view_pembelian`
--

DROP TABLE IF EXISTS `view_pembelian`;
/*!50001 DROP VIEW IF EXISTS `view_pembelian`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `view_pembelian` AS SELECT
 1 AS `id`,
  1 AS `tanggal`,
  1 AS `nomor`,
  1 AS `jumlah`,
...
--
-- Final view structure for view `view_pembelian`
--
...
/*!50001 VIEW `view_pembelian` AS select `pembelian`.`id` AS `id`,`pembelian`.`tanggal` AS `tanggal`,`pembelian`.`nomor` AS `nomor`,`pembelian`.`jumlah` AS `jumlah`,`pembelian`.`harga` AS `harga`,`produk`.`nama` AS `nama_produk`,`vendor`.`nama` AS `nama_vendor`,`vendor`.`kontak` AS `kontak_vendor` from ((`pembelian` left join `produk` on(`pembelian`.`id` = `produk`.`id`)) left join `vendor` on(`pembelian`.`id` = `vendor`.`id`)) */;
...
-- Dump completed on 2023-06-22 10:06:57
